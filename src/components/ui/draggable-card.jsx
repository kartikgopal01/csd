...
export const DraggableCardBody = ({ className, children }) => {
  ...
  const glareOpacity = useSpring(useTransform(mouseX, [-300, 0, 300], [0.1, 0, 0.1]), springConfig);
  ...
  return (
    <motion.div
      ...
      className={cn(
        "relative w-80 overflow-hidden rounded-xl bg-white shadow-2xl backdrop-blur-sm dark:bg-gray-800/90",
        className
      )}>
      {children}
      <motion.div
        ...
        className="pointer-events-none absolute inset-0 bg-gradient-to-br from-white/20 to-transparent select-none" />
    </motion.div>
  );
};
